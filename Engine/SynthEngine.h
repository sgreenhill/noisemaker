...
public:
	...
	SynthEngine(float sampleRate, bool withReverb=false) 
	{
		Params *params= new Params();
		this->param= params->parameters;
		initialize(sampleRate, withReverb);
	}
...
private:
	void initialize(float sampleRate, bool withReverb)
	{
		...
		reverbEngine = NULL;
		if (withReverb)
        	reverbEngine = new ReverbEngine(sampleRate);
...
public:
/*
	DO NOT USE - reinitialising without cleanup will leak resources
...
*/
    ...
    {
        chorusEngine->setEnablesChorus(isChorus1Enabled, isChorus2Enabled);
    }

    void setReverbWet(float value)
    {
		if (reverbEngine)
        	reverbEngine->setWet(value);
    }

    void setReverbDecay(float value)
    {
		if (reverbEngine)
        	reverbEngine->setDecayTime(value);
    }

    void setReverbPreDelay(float value)
    {
		if (reverbEngine)
        	reverbEngine->setPreDelay(value);
    }

    void setReverbHighCut(float value)
    {
        if (reverbEngine)
			reverbEngine->setHighCut(value);
    }

    void setReverbLowCut(float value)
    {
		if (reverbEngine)
        	reverbEngine->setLowCut(value);
    }
	...
	void process(float *sampleL, float *sampleR) 
	{
		...
		*sampleR = 0.0f;
		...
		if (reverbEngine)
	        reverbEngine->process(sampleL, sampleR);
	}
};
#endif
